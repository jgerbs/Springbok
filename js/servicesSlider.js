(() => {
    ...
    const shell = slider.closest(".services-v2-shell");
    if (!shell) return;

    const bar = shell.querySelector("[data-services-timer]");
    const nextBtn = shell.querySelector(".services-next");
    const prevBtn = shell.querySelector(".services-prev");

    const items = Array.from(slider.querySelectorAll(".services-item"));
